    ...
    return (
        <div className={'space-y-6'}>
            <div className={'rounded-xl bg-white p-6 shadow-sm border border-neutral-100 dark:bg-neutral-900 dark:border-neutral-800'}>
                <h3 className={'mb-4 text-xs font-bold uppercase tracking-wider text-neutral-500'}>Links</h3>
                <div className={'space-y-3'}>
                    {props.pkg.homepage && (
                        <a
                            ...
                            className={'flex items-center gap-3 text-sm font-medium text-neutral-600 transition-colors hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-neutral-200'}
                        >
            ...
            {props.pkg.maintainers && props.pkg.maintainers.length > 0 && (
                <div className={'rounded-xl bg-white p-6 shadow-sm border border-neutral-100 dark:bg-neutral-900 dark:border-neutral-800'}>
                    <h3 className={'mb-4 text-xs font-bold uppercase tracking-wider text-neutral-500'}>Maintainers</h3>
                            ...
                            <li key={maintainer.name}>
                                ...
                                >
                                    ...
                                    <div className={'flex flex-col'}>
                                        ...
                                        <span className={'text-xs text-neutral-400'}>{maintainer.email}</span>
                                    </div>
                                </a>
                            </li>
                        ))}
            ...
            {props.pkg.keywords && props.pkg.keywords.length > 0 && (
                <div className={'rounded-xl bg-white p-6 shadow-sm border border-neutral-100 dark:bg-neutral-900 dark:border-neutral-800'}>
                    <h3 className={'mb-4 text-xs font-bold uppercase tracking-wider text-neutral-500'}>Keywords</h3>
                    <div className={'flex flex-wrap gap-2'}>
                        {props.pkg.keywords.map((keyword) => (
                            <span
                                key={keyword}
                                className={'rounded-full bg-neutral-100 px-2.5 py-1 text-xs font-medium text-neutral-600 border border-neutral-200 dark:bg-neutral-800 dark:text-neutral-400 dark:border-neutral-700'}
                            >
                                {keyword}
                            </span>
                        ))}
